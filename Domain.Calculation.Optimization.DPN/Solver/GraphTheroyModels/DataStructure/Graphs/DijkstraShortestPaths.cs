...
namespace Algorithms.Graphs
{
        ...
        where TVertex : IComparable<TVertex>
    {
        ...
        private void _initializeDataMembers(TGraph Graph)
        {
            ...
            _distances = new decimal[_verticesCount];
        ...
        private bool _checkOptimalityConditions(TGraph graph, TVertex source)
        {
            ...
            // check that edge weights are nonnegative 不必要，删除
            ...
            foreach (var vertex in graph.Vertices)
            {
                ...
                foreach (var edge in graph.NeighboursMap(vertex))
                {
                    ...
                    if ((_distances[v] != Infinity)&&(_distances[v] + edge.Value < _distances[w]))//Bug fix: the enumable will skip v, if v is unreachable, 
                    {
        ...
        /// </summary>
        public decimal DistanceTo(TVertex destination)
        {
            if (!_nodesToIndices.ContainsKey(destination))
            {
                throw new Exception("Graph doesn't have the specified vertex.");
        ...
        public IEnumerable<TVertex> ShortestPathTo(TVertex destination)
        {
            ...
            int index;
            for (index = dstIndex; index != _srcIndex; index = _predecessors[index]) //替换终止条件 _distances[index] != 0
            {
